...
use crate::{error::Error, formats, http::http_client, install::{FetchResult, InstallContext}, primitives::{Ident, Locator}, semver};

use super::PackageData;

pub async fn fetch_locator<'a>(context: &InstallContext<'a>, locator: &Locator, ident: &Ident, version: &semver::Version, is_mock_request: bool) -> Result<FetchResult, Error> {
    ...
    let (archive_path, data, checksum) = context.package_cache.unwrap().upsert_blob_or_mock(is_mock_request, locator.clone(), ".zip", || async {
        ...
        formats::convert::convert_tar_gz_to_zip(ident, archive)
    }).await?;
